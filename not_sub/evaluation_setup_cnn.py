...
def get_predicted_labels():
	...
	_, documents_cl, labels = pp.extract(tokenized_dev, dev_labels, )
	...
	w2v = gensim.models.KeyedVectors.load_word2vec_format(const.google_pretrained_word2vec, binary=True)
	doc_generator = pp.doc_model_batch_gen(documents_cl, labels, batch_size, w2v)


    #load the model from memory. Note that this will be the only line that needs to change if we change models 
	mod = keras.models.load_model('dual_cnn_model.h5', custom_objects = {'tf':tf,'const':const})
	#mod = keras.models.load_model('dual_cnn_model.h5')
	...
	for i in range(0, len(documents_cl) - batch_size):
		max_prob = 0
		max_prob_label = 0
		for j in range(0, 11):
